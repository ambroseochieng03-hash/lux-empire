...
(function () {
    ...
    */
    ...
    CONFIRMATION MODAL
    =========================================
    Resolves true if the person confirms, false on Cancel / Esc / clicking
    outside. Reuses the .lux-booking-modal look. Used by the landlord
    Approve/Reject buttons and the tenant's Cancel/Delete booking buttons.
    */
    ...
    const CONFIRM_COPY = {
        ...
        cancel_booking: {
            title: 'Cancel your booking request?',
            message: 'The property becomes available to other tenants again and your booking fee is refunded to your M-Pesa automatically. This cannot be undone.',
        ...
        document.addEventListener('click', async (event) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // The request was already cancelled by the tenant, auto-declined
                    // after the response window, or handled in another tab — drop the
                    // stale card instead of leaving buttons that can never work.
                    const isStale = /already been handled|already been booked|not found/i.test(data.message || '');

                    if (isStale && card) {
                        card.remove();
                        maybeShowEmptyState();
                        showBookingModal((data.message || 'This request is no longer pending.') + ' It has been removed from your list.', 'error');
                        return;
                    }
    ...
    */
    ...
    function bindTenantBookingAjaxForms() {
        ...
        document.addEventListener('submit', async (event) => {
            ...
            const kind = /delete_booking/.test(form.action) ? 'delete_booking' : 'cancel_booking';

            const confirmed = await confirmAction(kind);
